...
class ReadingSessionModel extends BaseModel
{
    ...
    public function getRecentUserActivitySessions($userId, $limit = 5)
    {
        $sql = "
            SELECT 
                rs.rs_id,
     ...
     * 
     * @return array All reading sessions with user and book info
     */
    public function getAllReadingSessions()
    {
        $sql = "
            SELECT 
                ...
                CONCAT(ua.ua_first_name, ' ', ua.ua_last_name) as user_name,
                rs.b_id as book_id,
                b.b_title as book_title,
                b.b_author as book_author,
                rs.rs_started_at as start_time,
                rs.rs_expires_at as expiry_time,
                rp.last_updated as last_activity,
                rp.current_page as pages_read,
                b.b_pages as total_pages,
                CASE 
                    WHEN b.b_pages > 0 THEN 
                        ROUND((rp.current_page * 100.0) / b.b_pages) 
                    ELSE 0 
                END as completion_percentage,
                rp.is_completed as is_completed,
                CASE 
                    WHEN rs.rs_expires_at < NOW() THEN 'Expired'
                    WHEN rp.is_completed THEN 'Completed'
                    ELSE 'Active'
                END as status,
                CASE
                    WHEN up.up_id IS NOT NULL THEN TRUE
                    ELSE FALSE
                END as is_purchased,
                EXTRACT(EPOCH FROM (rp.last_updated - rs.rs_started_at))/60 as duration_minutes
            FROM 
                {$this->table} rs
            JOIN 
                books b ON rs.b_id = b.b_id
    ...
    /**
     * Get a specific reading session for admin view
     * 
     * @param int $sessionId Reading session ID
     * @return array|null Reading session with user and book info
     */
    public function getAdminReadingSession($sessionId)
    {
        $sql = "
            SELECT 
                rs.rs_id as id,
     ...
     * @return bool True on success, false on failure
     */
    public function deleteReadingSession($sessionId)
    {
        try {
            // First, delete the reading progress
            $sqlProgress = "DELETE FROM {$this->progressTable} WHERE rs_id = :session_id";
            $this->execute($sqlProgress, ['session_id' => $sessionId]);
            
            // Then, delete the reading session
            $sqlSession = "DELETE FROM {$this->table} WHERE rs_id = :session_id";
            $this->execute($sqlSession, ['session_id' => $sessionId]);
            
            return true;
        } catch (\Exception $e) {
            error_log("Error deleting reading session: " . $e->getMessage());
            return false;
        }
    }
} 