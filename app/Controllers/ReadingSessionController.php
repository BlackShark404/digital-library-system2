...
class ReadingSessionController extends BaseController
{
    ...
    /**
     * Get all reading sessions (Admin API)
     */
    public function getReadingSessions()
    {
        ...
        $readingSessions = $this->readingSessionModel->getAllReadingSessions();
        
        // Return the data as JSON
        $this->jsonSuccess(['data' => $readingSessions]);
    }
    
    /**
     * Get a specific reading session (Admin API)
     * 
     * @param int $id Reading session ID
     */
    public function getReadingSession($id)
    {
        // Check if user is admin
        if (!isset($_SESSION['user_id']) || !isset($_SESSION['is_admin']) || !$_SESSION['is_admin']) {
            $this->jsonError('Access denied', 403);
        ...
        $readingSession = $this->readingSessionModel->getAdminReadingSession($id);
        ...
        // Return the data as JSON
        $this->jsonSuccess(['data' => $readingSession]);
    }
    
    /**
     * Update a reading session (Admin API)
     * 
     * @param int $id Reading session ID
     */
    public function updateReadingSession($id)
    {
        // Check if user is admin
        if (!isset($_SESSION['user_id']) || !isset($_SESSION['is_admin']) || !$_SESSION['is_admin']) {
            $this->jsonError('Access denied', 403);
        ...
        if (!isset($data['current_page']) || !isset($data['is_completed'])) {
            $this->jsonError('Missing required fields', 400);
            return;
        }
        
        // Verify session exists
        $session = $this->readingSessionModel->getReadingSession($id);
        if (!$session) {
            $this->jsonError('Reading session not found', 404);
            return;
        }
        ...
        // Update reading progress
        $updated = $this->readingSessionModel->updateReadingProgress(
            $id, 
            intval($data['current_page']), 
        ...
        if ($updated) {
            ...
            $this->activityLogModel->logActivity(
                $_SESSION['user_id'], 
                'ADMIN_UPDATE', 
                'Updated reading session #' . $id
            );
            
            $this->jsonSuccess(['message' => 'Reading session updated successfully']);
        } else {
            $this->jsonError('Failed to update reading session', 500);
        }
    }
    
    /**
     * Delete a reading session (Admin API)
     * 
     * @param int $id Reading session ID
     */
    public function deleteReadingSession($id)
    {
        // Check if user is admin
        if (!isset($_SESSION['user_id']) || !isset($_SESSION['is_admin']) || !$_SESSION['is_admin']) {
            $this->jsonError('Access denied', 403);
        ...
        $deleted = $this->readingSessionModel->deleteReadingSession($id);
        ...
        if ($deleted) {
            ...
            $this->activityLogModel->logActivity(
                $_SESSION['user_id'], 
                'ADMIN_DELETE', 
                'Deleted reading session #' . $id
            );
            
            $this->jsonSuccess(['message' => 'Reading session deleted successfully']);
        } else {
            $this->jsonError('Failed to delete reading session', 500);
        }
    }
} 