        ...
        try {
            ...
            book.ready.then(() => {
                ...
                book.locations.generate(1024).then(() => {
                    // Get total pages from book or from server-stored value
                    const bookTotal = book.locations.total || 100;
                    // Use server value (from database) as a fallback, or the larger value for better granularity
                    totalPages = <?= $session['b_pages'] ?? 0 ?> > 0 ? 
                        Math.max(bookTotal, <?= $session['b_pages'] ?? 0 ?>) : 
                        bookTotal;
